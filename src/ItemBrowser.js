...
const FeedItem = React.forwardRef((props, ref) => (
		<ItemListRow read={props.item.status !== 'unread'} ref={ref} selected={props.selected}>
			<ItemListCell width='20px'>
				<Favico src={props.feed.icon_data} alt={props.feed.title} title={props.feed.title} />
			</ItemListCell>
...
function FeedItemList(props) {
	...
	const kbItemChange = (item, e) => {
		itemChange(item);
		if (selectedItem.current) selectedItem.current.scrollIntoView({block: 'center'});
		e.preventDefault();
	}
	useHotkeys('left,p,k', (e) => { 
		kbItemChange(props.items[(props.currentIndex - 1 >= 0) ? 
		(props.currentIndex - 1) :
		0], e);
	}, [props]);
	useHotkeys('right,n,j,space', (e) => {
		kbItemChange(props.items[(props.currentIndex + 1 < props.items.length) ? 
		(props.currentIndex + 1) : 
		((props.currentIndex < 0) ? 0 : (props.items.length - 1))], e);
	}, [props]);
	useHotkeys('home', (e) => {
	...
	useHotkeys('m', (e) => {
		if (props.currentIndex >= 0) props.markRead([props.items[props.currentIndex]], 
...
function FeedItemHeader(props) {
	...
	const markReadUntil = () => {
		if (props.currentIndex >= 0) props.markRead(props.items.slice(0, props.currentIndex + 1))
	};
	const markReadAfter = () => {
		if (props.currentIndex >= 0) props.markRead(props.items.slice(props.currentIndex))
	};
	useHotkeys('shift+A', () => markAllRead(), [props.items]);
	useHotkeys('u', () => props.setShowRead(!props.showRead), [props.showRead]);
	useHotkeys('s', () => props.setSortOldFirst(!props.sortOldFirst), [props.sortOldFirst]);
	...
	return (
		<Header>
			...
			<HeaderControls>
				<ControlButton onClick={markAllRead} title="Mark all as read"> &#10003; </ControlButton>
				<ControlButton onClick={markReadUntil} title="Mark all until selection as read"> &#11123; </ControlButton>
				<ControlButton onClick={markReadAfter} title="Mark all after selection as read"> &#11121; </ControlButton>
				<select
		  			value={props.showRead ? 'a' : 'u'}
		  			onChange={(v) => { props.setShowRead(v.target.value === 'a'); v.target.blur(); }}>
		  			<option value='u'>Show unread only</option>
		  			<option value='a'>Show all</option>
				</select>
				<select
		  			value={props.sortOldFirst ? 'o' : 'n'}
		  			onChange={(v) => { props.setSortOldFirst(v.target.value === 'o'); v.target.blur(); }}>
...
function ItemBrowser(props) {
	...
	useEffect(() => {
		const fetchItems = async () => {
			if (props.currentFeed) {
				...
				const urls = props.currentFeed.fetch_url ?
							...
							props.feeds
				 			.filter(f => f.category) 
							.filter(f => f.category.id === props.currentFeed.id).map(f => f.fetch_url);
				const result = await Promise.all(urls.map(u => 
					apiCall(u + 
						(u.includes('?') ? '&' : '?') +
						'limit=' + (parseInt(localStorage.getItem('fetch_limit')) || 100) +
						'&order=published_at&direction=' + (sortOldFirst ? 'asc' : 'desc') + 
						(showRead ? '' : '&status=unread'), props.errorHandler)
				  ));
				const items = [];
		...
		fetchItems();
	}, [sortOldFirst, showRead, props.currentFeed, props.feeds, props.errorHandler]);
	...
	const markRead = async (i, read) => {
		if (i.length > 0) {
			...
			i.forEach(item => 
				currItems[currItems.indexOf(item)].status = (read || read === undefined) ? 'read' : 'unread' 
			)
			setItems(currItems);
		}
	}
